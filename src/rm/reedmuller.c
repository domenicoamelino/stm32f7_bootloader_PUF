...
memory_vector *encode(memory_vector* v,int r,int m) {
	...
	vector *chunk, *partial;
    memory_vector *result;
	node *rmnode;
    ...
    result = createMemoryVector(num_chunks * generator_matrix->num_columns);
...
//            chunk->values[j] = v->values[pos_v];
            chunk->values[j] = getIntValue(v, pos_v);
        }
        ...
        for (j = 0; j < partial->length && pos_e < result->length; j++, pos_e++) {
            ...
            writeBitValue(result, partial->values[j], pos_e);
        }
...
memory_vector *decode(memory_vector *v, int r, int m) {
    int i, j, k;
    ...
    for (i = 0, pos_e = 0; i < num_chunks; i++) {
        ...
        for (j = 0; j < u_chunks[i]->length && pos_e < v->length; j++, pos_e++) {
            ...
            u_chunks[i]->values[j] = getIntValue(v, pos_e);
        }
    }
    ...
    result = createMemoryVector(num_chunks * partial_size);
...
//            result->values[pos_d] = r_chunks[i]->values[j];
            writeBitValue(result, r_chunks[i]->values[j], pos_d);
        }
