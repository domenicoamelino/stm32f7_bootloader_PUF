...
hex_loader_error_t intepretRecord(char* record, uint32_t length, hex_data* hex){
	if(!length || record[0] == '\n' || record[0] =='\r'){
	...
	if(sscanf(record+record_ptr, "%2x", &buf)!=1){
			return WRONG_FORMAT;
	}
	record_length = buf;
	record_ptr+=2;
	if(sscanf(record+record_ptr, "%4x", &address)!=1){
			return WRONG_FORMAT;
	}
	record_ptr+=4;
	if(sscanf(record+record_ptr, "%2x", &buf)!=1){
	...
	if(length >= record_length+record_ptr){
		...
		case DATA:
			...
			for(i=0; i<record_length; i++){
				if(sscanf(record+record_ptr, "%2x", memory_ptr)!=1){
					return WRONG_FORMAT;
				}
				memory_ptr+=1;
				record_ptr+=2;
